"""SDT encoder and classifiers extracted from ../SDT/model.py (appraisal_mode='none').

The temporal projections, speaker/position embeddings, nine transformers,
unimodal gates, feature reductions and classifier definitions are unchanged.
"""
import math
import torch
...
class MaskedKLDivLoss(nn.Module):
    ...
    def forward(self, log_pred, target, mask):
        ...
        loss = self.loss(log_pred[mask_], target[mask_]) / mask_.sum().float()
        return loss
...
class MaskedNLLLoss(nn.Module):
    ...
    def forward(self, pred, target, mask):
        ...
        if type(self.weight) == type(None):
            loss = self.loss(pred[mask_], target[mask_]) / mask_.sum().float()
        else:
            loss = self.loss(pred[mask_], target[mask_]) \
                   / torch.sum(self.weight[target[mask_]])
        return loss
...
class Multimodal_GatedFusion(nn.Module):
    ...
    def forward(self, *features, branch_mask=None):
        if len(features) < 2:
            raise ValueError('Multimodal_GatedFusion requires at least two features')
        utters = torch.stack(features, dim=-2)
        utters_fc = torch.stack([self.fc(feature) for feature in features],
                                dim=-2)
        if branch_mask is not None:
            branch_mask = branch_mask.unsqueeze(-1).type_as(utters_fc)
            utters_fc = utters_fc.masked_fill(branch_mask.eq(0), -1e10)
        utters_softmax = self.softmax(utters_fc)
        if branch_mask is not None:
            utters_softmax = utters_softmax * branch_mask
        utters_three_model = utters_softmax * utters
        final_rep = torch.sum(utters_three_model, dim=-2, keepdim=False)
        return final_rep

class SDTBackbone(nn.Module):
    def __init__(self, dataset, temp, D_text, D_visual, D_audio, n_head,
        ...
        self.speaker_embeddings = nn.Embedding(
            n_speakers + 1, hidden_dim, padding_idx=n_speakers)
